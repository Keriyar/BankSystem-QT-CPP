...
void bankdb::GetFile()
{
    ...
    ifstream iFile("E:\\QT\\bank-main\\bankdb.txt");
    ...
    for (int i = 0; i < iNumOfAccounts; i++)
    {
        ...
        getline(iFile,tmpAccount.identity);
        iFile >> iNumOfFlow;
...
void bankdb::WriteFile()
{
     ...
     */
    ofstream oFile("E:\\QT\\bank-main\\bankdb.txt");
 ...
 */
void bankdb::AddAccount(string strNum, string strNam, string strPas, string strAdd, int iTyp, double dInt, string strOpe,string strIde)
{
    ...
    tmpAccountInfo.identity=strIde;
...
string bankdb::GetIdentity(string strIdentity)
{
    return vAccount[mAccount[strIdentity]].identity;
...
string bankdb::GetIdendity(int iNum)
{
    return vAccount[iNum].identity;
}
/** 
