...
void accountlist::on_accountlist_searchEdit_textChanged(const QString &arg1)
{
    ...
    model->setHorizontalHeaderItem(7, new QStandardItem("身份呢"));
    ...
    for (int i = 0; i < iNumberOfAccounts; i++)
    {
        ...
        if (bankserver.GetNumber(i).find(strSearchString) != string::npos ||
            ...
            bankserver.GetIdentity(i).find(strSearchString) != string :: npos)
        {
            iItemsFinded++;
            ...
            model->setItem(iItemsFinded, 7, new QStandardItem(QString::fromStdString(bankserver.GetIdentity(i))));
        }
    }
...
void accountlist::SetDefault()
{
    ...
    model->setHorizontalHeaderItem(5, new QStandardItem("是否挂失"));
    model->setHorizontalHeaderItem(6, new QStandardItem("操作员"));
    model->setHorizontalHeaderItem(7, new QStandardItem("身份"));
    ...
    for (int i = 0; i < iNumberOfAccounts; i++)
    {
        ...
        string strIdentity = bankserver.GetIdentity(i);
        ...
        model->setItem(i, 7, new QStandardItem(QString::fromStdString(bankserver.GetIdentity(i))));
    }
}
