 ...
 */
bool bankServer::AccountAdd(string strNum, string strNam, string strPas, string strAdd, int iTyp, double dInt, string strOpe,string strIde)
{
    if (strNam != "" &&
        strPas != "" &&
        strAdd != "" &&
    ...
    {
        db.AddAccount(strNum, strNam, strPas, strAdd, iTyp, dInt, strOpe,strIde);
        return true;
    }
    return false;
}
...
string bankServer::GetIdentity()
{
    return db.GetIdentity(strCurrentNumber);
}
...
string bankServer::GetIdentity(int iNum)
{
    return db.GetIdendity(iNum);
}
/** 
