                                                ...
                                                ui(new Ui::createaccount)
{
    ...
    ui->login_accountInput->setStyleSheet("background:#F0F0F0;");//设置背景
    ui->login_interestInput->setStyleSheet("background:#F0F0F0;");
    ui->login_nameInput->setFocus();//获取焦点
...
void createaccount::on_login_okButton_clicked()//创建成功
{
    ...
    string strIde = ui->login_identityComboBox->currentText().toStdString();//将QString转换为string
    if (bankserver.AccountAdd(strNum, strNam, strPas, strAdd, iTyp, dInt, strOpe,strIde))
    {
